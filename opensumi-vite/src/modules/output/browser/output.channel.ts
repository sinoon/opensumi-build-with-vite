...
export class OutputChannel extends Disposable {
    ...
    constructor(@Optional() public readonly name: string) {
        ...
        const preferenceMaxLine = this.preferenceService.get<number>(
        ...
        this.enableHighlight = Boolean(
            this.preferenceService.get<boolean>('output.enableLogHighlight'),
        )
        ...
        const uri = new URI(
            `${Schemes.walkThroughSnippet}://output/${name || uuid()}`,
        )
        this.documentService.createModelReference(uri).then(model => {
            ...
            this.monacoModel.setValue(
                localize('output.channel.none', '还没有任何输出'),
            )
    ...
    private setShouldLogToBrowser() {
        // const noVisiblePanel =
        //     !this.layoutService.getTabbarHandler('ide-output')
    ...
    append(value: string): void {
        this.eventBus.fire(
            new ContentChangeEvent(
                new ContentChangeEventPayload(
                    this.name,
                    ContentChangeType.append,
                    value,
                    this.outputLines,
                ),
            ),
