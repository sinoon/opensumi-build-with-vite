...
export const Output = observer(({ viewState }: { viewState: ViewState }) => {
    ...
    useEffect(() => {
        if (outputRef.current) {
            outputService.createOutputChannel(outputRef.current, 'output')
        }
    }, [])
...
export const ChannelSelector = observer(() => {
    ...
    async function handleChange(
        ...
        if (channelName !== NONE) {
            outputService.updateSelectedChannel(
                outputService.getChannel(channelName),
            )
        }
    }
        ...
        <NativeSelect
            value={
                outputService.selectedChannel
                    ? outputService.selectedChannel.name
                    : NONE
            }
