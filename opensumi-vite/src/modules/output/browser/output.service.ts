...
export class OutputService extends WithEventBus {
    ...
    constructor() {
        ...
        this.enableSmartScroll = Boolean(
            this.preferenceService.get<boolean>('output.enableSmartScroll'),
        )
        this.addDispose(
            this.preferenceService.onPreferenceChanged(e => {
                if (
                    e.preferenceName === 'output.enableSmartScroll' &&
                    e.newValue !== this.enableSmartScroll
                ) {
    ...
    public createOutputChannel(el: HTMLElement, name: string, options?: {}) {
        el.className = styles.output
        // create Editor Instance
        this.initOutputMonacoInstance(el)
        // create channel
    ...
    public updateSelectedChannel(channel: OutputChannel) {
        ...
        this.selectedChannel.modelReady.promise.then(() => {
            const model =
                this.selectedChannel.outputModel.instance.getMonacoModel()
            ...
            this.monacoDispose = model.onDidChangeContent(() => {
                if (this.autoReveal && this.enableSmartScroll) {
                    this.outputEditor?.monacoEditor.revealLine(
                        model.getLineCount(),
                        0,
                    )
                }
    ...
    private async initOutputMonacoInstance(container: HTMLElement) {
        this.outputEditor = this.editorCollectionService.createCodeEditor(
            container,
            {
                ...getSimpleEditorOptions(),
        ...
        this.addDispose(
            this.outputEditor.monacoEditor.onMouseUp(e => {
                ...
                if (this.enableSmartScroll) {
                    ...
                    const maxLine = this.outputEditor?.currentDocumentModel
                        ?.getMonacoModel()
                        .getLineCount()
